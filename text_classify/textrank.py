...
class TextRank:
    def __init__(self,text=None):
        self.text = text
        if self.text:
            self.keyphrases = self.extractKeyphrases(text)
            self.summary = self.extractSentences(text)
    
    #apply syntactic filters based on POS tags
    def _filter_for_tags(self,tagged, tags=['NN', 'JJ', 'NNP']):
    ...
    def extractKeyphrases(self,text):
        ...
        tagged = self._filter_for_tags(tagged)
    ...
    def extractSentences(self,text):
        ...
        graph = self._buildGraph(sentenceTokens)
